<script>
	...
	async function loadNotes() {
		...
		try {
			...
			if (response.ok) {
				const data = await response.json();
				notes = data.notes; // Notes will now include timestamp
			} else {
				const errorData = await response.json();
...
<div class="container mx-auto p-4">
	...
	<div class="text-center">
		...
		<p class="text-gray-600 text-sm sm:text-base">
			Write your thoughts, reflect on your day, and grow personally.
		</p>
	</div>
	...
	<div class="container mx-auto p-4">
		...
		<div class="mt-8">
			...
			{#if notes.length > 0}
				<button class="hover:text-blue-900 text-blue-500 font-bold mb-4" on:click={hideAllNotes}>
					Hide All Entries
				</button>
			{:else}
				<button class="hover:text-blue-900 text-blue-500 font-bold mb-4" on:click={unhideAllNotes}>
					Unhide All Entries
				</button>
			{/if}
